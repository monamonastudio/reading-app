<template>
  ...
  <div class="p-4 flex justify-between">
    ...
    <NuxtLink class=" text-purple-400 font-bold tracking-wide">View All</NuxtLink>
  </div>
  ...
  <div class="grid grid-flow-col overflow-auto p-4 gap-4">
    ...
    <div class="w-[280px] h-[350px] relative rounded-xl overflow-hidden select-none" @click="navigateTo(`/book/${book.id}`)" v-for="i in 4">
      ...
      <div class="w-[280px] h-[350px] absolute z-10 bg-gradient-to-t from-slate-950 via-transparent">
        <div class="border-t-4 border-purple-300 bg-black/25 absolute bottom-0 h-32 py-2 px-4">
          <h2 class="text-xl font-semibold">The Procrastination Guide: Answers You Seek</h2>
          <p class="my-2">Author: John Doe</p>
        </div>
      </div>
    </div>
  </div>
